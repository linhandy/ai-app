...
async function generateVoiceover(script, outputPath) {
  ...
  await tts.setMetadata(CONFIG.ttsVoice, OUTPUT_FORMAT.AUDIO_24KHZ_96KBITRATE_MONO_MP3, meta);

  // -20% rate: relaxed pacing matching reference vid (~46s for 80 words)
  const ssml = `<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" xml:lang="en-US"><voice name="${CONFIG.ttsVoice}"><prosody rate="-20%">${script}</prosody></voice></speak>`;

  // rawToFile saves audio.mp3 + metadata.json with real WordBoundary events
  const workDir = path.dirname(outputPath);
  const tmpDir  = path.join(workDir, '_tts_tmp');
  mkdirSync(tmpDir, { recursive: true });

  const { audioFilePath, metadataFilePath } = await tts.rawToFile(tmpDir, ssml);

  // Move audio to final path
...
async function renderVideo({ images, audioPath, words, audioDuration, outputPath }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const assPath = path.join(CONFIG.outputDir, 'captions.ass');
    ...
    // ASS subtitles filter (uses Windows font system, not fontconfig)
    const assEscaped = assPath.replace(/\\/g, '/').replace(/:/g, '\\:');
    const subtitlesFilter = `[vconcat]subtitles='${assEscaped}':fontsdir='C\\:/Windows/Fonts'[vout]`;
