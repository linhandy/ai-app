...
export default function ComparePanel({ beforeUrl, afterUrl, style }: Props) {
  return (
    ...
    {/* Side-by-side direct comparison */}
      <div className={beforeUrl ? "grid grid-cols-2 gap-3 md:gap-5" : ""}>
        {/* Before — only shown if beforeUrl is available */}
        {beforeUrl && (
        ...
        {/* After — expands to full width if no before image */}
        <div className={`relative rounded-xl overflow-hidden border border-amber-500/50 ${!beforeUrl ? 'max-w-2xl mx-auto' : ''}`}>
