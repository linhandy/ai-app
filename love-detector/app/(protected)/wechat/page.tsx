...
export default function WechatPage() {
    ...
    reader.onload = async (ev) => {
      ...
      try {
        ...
        if (data.text) {
          ...
          setOcrError(data.error || '识别失败，请重试或改用文字粘贴')
        }
      } catch {
  ...
  async function handleAnalyze() {
    ...
    const content = inputMode === 'text' ? text
      : inputMode === 'image' ? ocrText
      : `[语音文件: ${voiceFile?.name}]`

    try {
      const res = await fetch('/api/analyze-wechat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ content, source: inputMode }),
      })
      const data = await res.json()
      setAnalysis(data)
    } catch {
  ...
  function buildShareText() {
    ...
    return [
      '【恋爱测谎仪 · 微信记录分析报告】',
      `综合可信度：${analysis.credibilityScore}%`,
      `${LEVEL_CFG[analysis.level]?.emoji ?? ''} ${analysis.levelLabel}`,
      '',
      analysis.summary,
      '',
      '📚 理论依据：',
      analysis.theoreticalBasis,
      '',
      '💡 建议：',
      analysis.advice,
      '',
      ...
      <div className="flex flex-col min-h-screen bg-gray-50">
        ...
        <div className="flex-1 px-5 py-4 space-y-4 overflow-y-auto">
          ...
          <div className="bg-white rounded-2xl p-1 flex shadow-sm">
            {([['text', '💬 粘贴文字'], ['image', '🖼 截图上传'], ['voice', '🔊 语音文件']] as const).map(([val, label]) => (
              <button
                key={val}
                onClick={() => { setInputMode(val); setAnalysis(null) }}
          ...
          {inputMode === 'text' && (
            <div className="bg-white rounded-2xl p-4 shadow-sm">
              <textarea
                ...
                placeholder={'在这里粘贴 TA 的消息内容...\n\n例如：\n"我今晚要加班"\n"没有删过聊天记录"\n"只是普通朋友"'}
                className="w-full min-h-[160px] text-sm text-gray-800 placeholder-gray-400 outline-none resize-none border-2 border-purple-100 rounded-xl p-3 focus:border-purple-400 bg-purple-50/20"
              />
            ...
            <div className="bg-white rounded-2xl p-4 shadow-sm space-y-3">
              ...
              {imagePreview ? (
                <div className="relative">
                  ...
                  <button
                    onClick={() => { setImagePreview(null); setOcrText(''); setOcrError(''); setAnalysis(null); if (imageInputRef.current) imageInputRef.current.value = '' }}
                ...
                <button
                  ...
                  className="w-full py-12 border-2 border-dashed border-purple-200 rounded-xl flex flex-col items-center gap-2 text-purple-400 active:bg-purple-50 transition-colors"
                >
              ...
              {ocrLoading && (
                <div className="flex items-center gap-2 py-1">
                  ...
                  <span className="text-purple-600 text-xs">正在识别截图文字...</span>
                </div>
              )}
              {ocrError && <p className="text-red-500 text-xs">{ocrError}</p>}
              {ocrText && (
                <div className="bg-blue-50 rounded-xl p-3 border border-blue-100">
                  <p className="text-[10px] text-blue-500 font-semibold mb-1.5">📄 识别到的文字内容：</p>
                  <p className="text-gray-700 text-xs leading-relaxed max-h-32 overflow-y-auto">{ocrText}</p>
                </div>
              )}
            </div>
          )}
          ...
          {inputMode === 'voice' && (
            <div className="bg-white rounded-2xl p-4 shadow-sm">
              <input
                ...
                accept="audio/*"
                className="hidden"
                onChange={e => { setVoiceFile(e.target.files?.[0] ?? null); setAnalysis(null) }}
              />
              <div
                className={`border-2 border-dashed rounded-2xl p-8 flex flex-col items-center gap-3 cursor-pointer transition-colors ${
                  voiceFile ? 'border-purple-400 bg-purple-50' : 'border-gray-200 bg-gray-50'
                }`}
              ...
              >
                {voiceFile ? (
                  <>
                    ...
                    <p className="text-purple-700 font-semibold text-sm">{voiceFile.name}</p>
                    <p className="text-gray-400 text-xs">{(voiceFile.size / 1024).toFixed(1)} KB</p>
                    <button onClick={e => { e.stopPropagation(); setVoiceFile(null) }} className="text-red-400 text-xs">删除</button>
                  </>
                ) : (
                  <>
                    <span className="text-4xl">🔊</span>
                    <p className="text-gray-600 font-semibold text-sm">点击上传语音文件</p>
                    <p className="text-gray-400 text-xs">支持 MP3、M4A、WAV 格式</p>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="space-y-4">
                  ...
                  <div className="relative w-20 h-20 shrink-0">
                    <svg width="80" height="80" className="-rotate-90">
                      ...
                      <circle
                        ...
                        stroke={analysis.levelColor === 'green' ? '#059669' : analysis.levelColor === 'blue' ? '#2563EB' : analysis.levelColor === 'yellow' ? '#F59E0B' : analysis.levelColor === 'orange' ? '#F97316' : '#EF4444'}
                        strokeWidth="7" strokeLinecap="round"
                        strokeDasharray={`${2 * Math.PI * 32}`}
                        strokeDashoffset={`${2 * Math.PI * 32 * (1 - analysis.credibilityScore / 100)}`}
                      />
                    </svg>
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-lg font-bold text-gray-900">{analysis.credibilityScore}%</span>
                    </div>
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xl">{LEVEL_CFG[analysis.level]?.emoji}</span>
                      <span className={`font-bold text-sm ${LEVEL_CFG[analysis.level]?.text}`}>{analysis.levelLabel}</span>
                    </div>
                      ...
                      return (
                        <div key={i} className="border border-gray-100 rounded-xl p-3">
                          <div className="flex items-start gap-2 mb-1.5">
                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full shrink-0 ${rc.bg} ${rc.text}`}>{rc.label}</span>
                            <p className="text-gray-800 text-xs font-medium">「{sp.quote}」</p>
                          </div>
                          <p className="text-gray-500 text-xs leading-relaxed">{sp.reason}</p>
                        </div>
                      )
                    })}
                  </div>
              ...
              <div className="rounded-2xl overflow-hidden shadow-sm">
                <div className="bg-[#1E1035] px-4 py-3 flex items-center gap-2">
                  ...
                  <h3 className="text-white font-bold text-sm">AI 深度分析</h3>
                </div>
                <div className="bg-[#2A1A50] px-4 py-4 space-y-4">
                  <div className="bg-white/10 rounded-xl p-3">
                    <p className="text-purple-300 text-[10px] font-semibold mb-1.5">📚 理论依据</p>
                    <p className="text-white/80 text-xs leading-relaxed">{analysis.theoreticalBasis}</p>
                  </div>
                  {analysis.patterns?.length > 0 && (
                    <div>
                      <p className="text-purple-300 text-[10px] font-semibold mb-2">📊 语言模式</p>
                      <div className="space-y-1.5">
                  ...
                  <div className="bg-white/10 rounded-xl p-3">
                    ...
                    <p className="text-white/80 text-xs leading-relaxed">{analysis.advice}</p>
                  </div>
          ...
          {!analyzing && !analysis && (
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              <p className="text-amber-700 text-xs leading-relaxed">
                ⚠️ 本工具仅供娱乐参考，基于语言心理学模型分析，不构成事实判断。请理性对待结果，重要问题建议直接沟通。
              </p>
            </div>
          )}
        </div>
        ...
        {analysis && (
          <div className="px-5 pb-2 bg-gray-50">
            <button
              onClick={handleShare}
              className="w-full py-4 rounded-2xl text-white font-bold text-base gradient-primary shadow-lg shadow-purple-500/30"
            >
              💬 分享到微信
            </button>
          </div>
